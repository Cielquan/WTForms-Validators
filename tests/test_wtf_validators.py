...
class TestDecimalDigits(object):
    ...
    @pytest.mark.parametrize(
        ...
        (
            (DecimalDigits(3, -1), 'foo', """Data type "<class 'str'>" is not a supported."""),
            ...
            (DecimalDigits(3, 4), 0.12, "Number must have between 3 and 4 decimal digits.")
        )
    )
    def test_decimal_digits_messages(self, dummy_form, dummy_field, validator, data, message):
        """
...
class TestEqualStateTo(object):
    ...
    def test_test_equal_state_to_error_messages(self, dummy_form, cbx_ticked, cbx_unticked):
        """
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (EqualStateTo('foo'), "'Checkbox_false' and 'Checkbox_true' must be of equal state."),
            (EqualStateTo('foo', message="foo"), "foo"),
        )
    )
    def test_test_equal_state_to_error_messages(self, dummy_form, cbx_ticked, cbx_unticked, validator, message):
        """
...
class TestInputRequiredIfCheckbox(object):
    ...
    def test_input_required_if_checkbox_raises(self, dummy_form, cbx_unticked, input_filled):
        """
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (InputRequiredIfCheckbox('foo'), "'Input_empty' is required for 'Checkbox_true'."),
            (InputRequiredIfCheckbox('foo', message="foo"), "foo"),
        )
    )
    def test_input_required_if_checkbox_error_message(self, dummy_form, cbx_ticked, input_empty, validator, message):
        """
        It should return error message when the required value is not present when the checkbox is ticked.
        """
        dummy_form['foo'] = cbx_ticked
